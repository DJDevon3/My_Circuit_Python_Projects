...
Save settings.toml replacing Next Refresh Token with Initial Refres Token manually, then save code.py with new changes
I will be looking for a way to automate this process in the future with file appends.

Fitbit Token only needs to be generated by the tutorial once as long as the refresh tokens are automatically updating during normal script run.
